...
public sealed class ExecuteReviewFailedStateChangeCommandHandler : IRequestHandler<ExecuteReviewFailedStateChangeCommand, Unit>
{
    ...
    /// <inheritdoc />
    public async Task<Unit> Handle(ExecuteReviewFailedStateChangeCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!issue.Execute(new ReviewFailedStateChangeCommand()))
        {
